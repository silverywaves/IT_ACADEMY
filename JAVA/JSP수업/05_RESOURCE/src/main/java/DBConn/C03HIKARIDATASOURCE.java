...
@WebServlet("/Dbtest_hikaricp")		// 서블릿 클래스를 /Dbtest_hikaricp URL 패턴에 매핑
public class C03HIKARIDATASOURCE extends HttpServlet {	// C03HIKARIDATASOURCE 클래스 선언, HttpServlet 클래스 상속
	...
	public void init() throws ServletException {
		try {
			...
			dataSource = (DataSource)envContext.lookup("jdbc/HikariDataSource"); // JNDI 이름 "jdbc/HikariDataSource"를 사용하여 DataSource 검색
			conn = dataSource.getConnection();	// DataSource를 사용하여 데이터베이스 연결
			System.out.println("Connection : " + conn);	// 연결된 데이터베이스 정보를 콘솔에 출력
			
		} catch(Exception e) {	// 예외가 발생할 경우 예외 정보를 출력
			e.printStackTrace();
		}
	}
	...
	/*	[HikariCP 데이터베이스 연결 풀을 사용하여 데이터베이스에 연결]
