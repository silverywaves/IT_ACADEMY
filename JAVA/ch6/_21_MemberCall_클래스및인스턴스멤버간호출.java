...
public class _21_MemberCall_클래스및인스턴스멤버간호출 {
	int iv = 10;
	static int cv = 20;
	
	int iv2 = cv;
//	static int cv2 = iv;	// error : 클래스변수는 인스턴스 변수 사용 불가
	static int cv2 = new _21_MemberCall_클래스및인스턴스멤버간호출().iv;	// 객체 생성해야 사용 가능
	
	static void staticMethod1() {
		System.out.println(cv);
//		System.out.println(iv);	// error : 클래스메서드에서 인스턴스변수 사용 불가
		_21_MemberCall_클래스및인스턴스멤버간호출 c = new _21_MemberCall_클래스및인스턴스멤버간호출();
