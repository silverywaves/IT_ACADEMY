package springbin.componentScan.service;
...
@Service
public class MemberService {    // MemberService는 순수한 자바 클래스 -> 스프링이 이 클래스를 알 수 있는 방법이 없음
    // MemberController는 애노테이션을 걸어두어 스프링이 뜰 때 "내가 관리하는 것이니 생성해야겠다"가 되는데, MemberService는 안되는 상황
    // => 애노테이션 걸어줌(@Service / @Autowired) -> MemoryMemberRepository 에는 @Repository 걸어주기
